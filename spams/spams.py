...
#    paramlist = [('thrs',1.0),('mode',1),('lambda1',0.0),('lambda2',0.0),('lambda3',0.0),('pos',0),('numThreads',-1)]
    ...
    spams_wrap.sparseProject(U, V, thrs, mode, lambda1,
                             lambda2, lambda3, pos, numThreads)
    return V
...
#                 ('max_length_path', -1),('verbose',True),('cholesky', False)]
    ...
    if q is not None:
        if return_reg_path:
            ((indptr, indices, data, shape), path) = spams_wrap.lassoQq(X, Q, q, return_reg_path,
                                                                        L, lambda1, lambda2, mode, pos, ols, numThreads, max_length_path, verbose, cholesky)
        else:
            (indptr, indices, data, shape) = spams_wrap.lassoQq(X, Q, q, return_reg_path, L,
                                                                lambda1, lambda2, mode, pos, ols, numThreads, max_length_path, verbose, cholesky)
    else:
        if return_reg_path:
            ((indptr, indices, data, shape), path) = spams_wrap.lassoD(X, D, return_reg_path, L,
                                                                       lambda1, lambda2, mode, pos, ols, numThreads, max_length_path, verbose, cholesky)
        else:
...
def lassoMask(X, D, B, L=-1, lambda1=None, lambda2=0.,
    ...
    (indptr, indices, data, shape) = spams_wrap.lassoMask(
        X, D, B, L, lambda1, lambda2, mode, pos, numThreads, verbose)
    alpha = ssp.csc_matrix((data, indices, indptr), shape)
    return alpha
...
def lassoWeighted(X, D, W, L=-1, lambda1=None,
    ...
    (indptr, indices, data, shape) = spams_wrap.lassoWeighted(
        X, D, W, L, lambda1, mode, pos, numThreads, verbose)
    alpha = ssp.csc_matrix((data, indices, indptr), shape)
    return alpha
...
def omp(X, D, L=None, eps=None, lambda1=None, return_reg_path=False, numThreads=-1):
    ...
    if return_reg_path:
        ((indptr, indices, data, shape), path) = spams_wrap.omp(
            X, D, return_reg_path, given_L, L, given_eps, eps, given_lambda1, lambda1, numThreads)
    else:
        (indptr, indices, data, shape) = spams_wrap.omp(X, D, return_reg_path,
                                                        given_L, L, given_eps, eps, given_lambda1, lambda1, numThreads)
    alpha = ssp.csc_matrix((data, indices, indptr), shape)
    if return_reg_path:
        return (alpha, path)
    else:
...
def ompMask(X, D, B, L=None, eps=None, lambda1=None, return_reg_path=False, numThreads=-1):
    ...
    if lambda1 is None:
        ...
        if str(type(lambda1)) != "<type 'numpy.ndarray'>":
            lambda1 = np.array([lambda1], dtype=X.dtype)
    if return_reg_path:
        ((indptr, indices, data, shape), path) = spams_wrap.ompMask(
            X, D, B, return_reg_path, given_L, L, given_eps, eps, given_lambda1, lambda1, numThreads)
    else:
        (indptr, indices, data, shape) = spams_wrap.ompMask(X, D, B, return_reg_path,
                                                            given_L, L, given_eps, eps, given_lambda1, lambda1, numThreads)
    alpha = ssp.csc_matrix((data, indices, indptr), shape)
    if return_reg_path:
        return (alpha, path)
...
def cd(X, D, A0, lambda1=None, mode=spams_wrap.PENALTY, itermax=100, tol=0.001, numThreads=-1):
    ...
    (indptr, indices, data, shape) = spams_wrap.cd(
        X, D, A0, lambda1, mode, itermax, tol, numThreads)
    alpha = ssp.csc_matrix((data, indices, indptr), shape)
    return alpha
...
def somp(X, D, list_groups, L=None, eps=0., numThreads=-1):
    ...
    (indptr, indices, data, shape) = spams_wrap.somp(
        X, D, list_groups, L, eps, numThreads)
    alpha = ssp.csc_matrix((data, indices, indptr), shape)
    return alpha
...
def l1L2BCD(X, D, alpha0, list_groups, lambda1=None, mode=spams_wrap.PENALTY, itermax=100, tol=1e-3, numThreads=-1):
    ...
    spams_wrap.l1L2BCD(X, D, alpha, list_groups, lambda1,
                       mode, itermax, tol, numThreads)
    return alpha
...
#    W = np.empty((W0.shape[0],W0.shape[1]),dtype=W0.dtype,order="F")
    ...
    optim_info = spams_wrap.fistaFlat(Y, X, W0, W, groups, numThreads, max_it, L0, fixed_step, gamma, lambda1, delta, lambda2, lambda3, a, b, c, tol, it0, max_iter_backtracking, compute_gram,
                                      lin_admm, admm, intercept, resetflow, regul, loss, verbose, pos, clever, log, ista, subgrad, logName, is_inner_weights, inner_weights, size_group, sqrt_step, transpose, linesearch_mode)
...
def fistaTree(
    ...
    W = np.zeros((W0.shape[0], W0.shape[1]), dtype=W0.dtype, order="F")
    optim_info = spams_wrap.fistaTree(Y, X, W0, W, eta_g, groups, own_variables, N_own_variables, numThreads, max_it, L0, fixed_step, gamma, lambda1, delta, lambda2, lambda3, a, b, c, tol, it0, max_iter_backtracking,
                                      compute_gram, lin_admm, admm, intercept, resetflow, regul, loss, verbose, pos, clever, log, ista, subgrad, logName, is_inner_weights, inner_weights, size_group, sqrt_step, transpose, linesearch_mode)
    if return_optim_info:
        return(W, optim_info)
    else:
        return W
...
def fistaGraph(
    ...
    W = np.zeros((W0.shape[0], W0.shape[1]), dtype=W0.dtype, order="F")
    optim_info = spams_wrap.fistaGraph(Y, X, W0, W, eta_g, groups, groups_var, numThreads, max_it, L0, fixed_step, gamma, lambda1, delta, lambda2, lambda3, a, b, c, tol, it0, max_iter_backtracking,
                                       compute_gram, lin_admm, admm, intercept, resetflow, regul, loss, verbose, pos, clever, log, ista, subgrad, logName, is_inner_weights, inner_weights, size_group, sqrt_step, transpose, linesearch_mode)
    if return_optim_info:
        return(W, optim_info)
    else:
...
#    params = __param_struct(paramlist,param)
    ...
    val_loss = spams_wrap.proximalFlat(U, alpha, groups, numThreads, lambda1, lambda2, lambda3,
                                       intercept, resetflow, regul, verbose, pos, clever, eval, size_group, transpose)
    if return_val_loss:
...
#    params = __param_struct(paramlist,param)
    ...
    if(len(tree) != 4):
        raise ValueError(
            "proximalTree : tree should be a named list of 4 elements")
    eta_g = tree['eta_g']
    groups = tree['groups']
    own_variables = tree['own_variables']
    N_own_variables = tree['N_own_variables']
    val_loss = spams_wrap.proximalTree(U, alpha, eta_g, groups, own_variables, N_own_variables, numThreads,
                                       lambda1, lambda2, lambda3, intercept, resetflow, regul, verbose, pos, clever, eval, size_group, transpose)
    if return_val_loss:
        return(alpha, val_loss)
    else:
        return alpha
...
def proximalGraph(U, graph, return_val_loss=False, numThreads=-1, lambda1=1.0, lambda2=0.,
    ...
    if(len(graph) != 3):
        raise ValueError(
            "proximalGraph : tree should be a named list of 3 elements")
    eta_g = graph['eta_g']
    groups = graph['groups']
    groups_var = graph['groups_var']
    alpha = np.zeros((U.shape[0], U.shape[1]), dtype=U.dtype, order="F")
    val_loss = spams_wrap.proximalGraph(U, alpha, eta_g, groups, groups_var, numThreads, lambda1,
                                        lambda2, lambda3, intercept, resetflow, regul, verbose, pos, clever, eval, size_group, transpose)
    if return_val_loss:
        return(alpha, val_loss)
    else:
        return alpha
...
def __allTrainDL(X, return_model=None, model=None, in_memory=False,
    ...
    if tree is None and graph is None:
        ...
        groups = ssp.csc_matrix(
            np.array([[False], [False]], dtype=np.bool_, order="F"))
    if tree is not None:
        ...
        if graph is not None:
            raise ValueError(
                "structTrainDL : only one of tree or graph can be given")
        eta_g = tree['eta_g']
        groups = tree['groups']
        own_variables = tree['own_variables']
        N_own_variables = tree['N_own_variables']
...
def archetypalAnalysis(X, p=10, Z0=None, returnAB=False, robust=False, epsilon=1e-3, computeXtX=False, stepsFISTA=3, stepsAS=50, randominit=False, numThreads=-1):
    ...
    if Z0 is None:
        (Z, (indptr, indices, data, shape), (indptr2, indices2, data2, shape2)) = spams_wrap.archetypalAnalysis(
            X, p, robust, epsilon, computeXtX, stepsFISTA, stepsAS, randominit, numThreads)
    else:
        (Z, (indptr, indices, data, shape), (indptr2, indices2, data2, shape2)) = spams_wrap.archetypalAnalysisInit(
            X, Z0, robust, epsilon, computeXtX, stepsFISTA, stepsAS, numThreads)
    if (returnAB):
...
def decompSimplex(X, Z, computeXtX=False, numThreads=-1):
    ...
    (indptr, indices, data, shape) = spams_wrap.decompSimplex(
        X, Z, computeXtX, numThreads)
    alpha = ssp.csc_matrix((data, indices, indptr), shape)
    return alpha
...
def treeOfGroupStruct(gstruct):
    ...
    (nbvars, perm, eta_g, groups, own_variables,
     N_own_variables) = spams_wrap.treeOfGroupStruct(gstruct)
    (indptr, indices, data, shape) = groups
    groups = ssp.csc_matrix((data, indices, indptr), shape)
